...
def kalman_out_to_pandas_for_map(out_kalman):
    ...
    for frame_data in out_kalman:
        ...
        for track in frame_data[1]:
            ...
            frame_vals = [img_id, boxes, track_id, scores, time_stamp]

            vals.append(frame_vals)

    df_kalman = pd.DataFrame(vals, columns=['img_id', 'boxes', 'track_id', 'scores', 'time_stamp'])
...
def obtain_timeoff_fps(tstamp_path, camera):
